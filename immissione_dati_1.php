	...
	<div class="Compose-Message">               
                <div class="panel panel-success">
                    <div class="panel-heading">
                       Jornal O clima (Metodo Simplex) parte 2 - Inserindo os dados
                    </div>
...
for ($j = 0; $j < $numVariables; $j++) {
	$content.= sprintf("<input type=\"text\" name=\"c[%d]\" size=\"8\" 
maxlength=\"8\"> x<sub>%d</sub> +\n", $j + 1, $j + 1);
}
// ... d.
$content.= ' <input type="text" name="d" size="8" maxlength="8" style="display:none"><br><br>
	Restrições: <br><br>
	';
...
for ($i = 0; $i < $numConstraints; $i++) {
	...
	$content.= sprintf("<input type=\"text\" name=\"a[%d][1]\" size=\"8\" 
maxlength=\"8\"> x<sub>1</sub>\n", $i + 1); // Le altre $numVariables variabili.
	for ($j = 1; $j < $numVariables; $j++) {
		$content.= sprintf("+ <input type=\"text\" name=\"a[%d][%d]\" size=\"8\" 
maxlength=\"8\"> x<sub>%d</sub>\n", $i + 1, $j + 1, $j + 1);
	}
	$content.= sprintf("<select 
name=\"lge[%d]\"><option>=&lt;<option>&gt;=<option>=</select> <input 
type=\"text\" name=\"b[%d]\" size=\"8\" maxlength=\"8\"><br>\n", $i + 1, $i + 1);
}
// non negativita'
$content.= '
	&nbsp; &nbsp; x<sub>i</sub> &gt;= 0';
// ed eventuale interezza
if (isset($intera) && !strcmp($intera, "true")) $content.= ' e';
...
$content.= '</strong><br>
        <table border="0" summary="invio form">
          <thead>
            <tr>
              <th><input type="button"  style="border:1px solid transparent; border-radius:15px; " value=" Finalizar! " class="btn btn-success"
