...
// $base_df[]        L'insieme degli 
 base nella fase 1 del metodo delle 2 fasi
//
     ...
     <div class="Compose-Message">               
                <div class="panel panel-success">
                    <div class="panel-heading">
                       Jornal O clima (Metodo Simplex) Resultados
                    </div>
                   ';
...
function riduci_standard(&$minmax, &$numVariables, $numConstraints, &$a, &$b, &$c, &$d, &$lge, &$intera, &$cambia_segno, &$base) {
	...
	if (strcmp($minmax, "min")) {
		...
		$d->negatefrac();
		$minmax = "min";
		$tmp = "&Egrave; o problema foi alterado de máximo para mínimo mudar o sinal de <b>z</b>.<br>\n";
	} //strcmp( $minmax, "min" )
	...
	for ($i = 1; $i < $numConstraints + 1; $i++) {
		if (!strcmp($lge[$i], "=<")) {
			...
			$lge[$i] = "=";
			$tmp.= "Introduziu a variável <em>negligente</em> x<sub>$j</sub> na fila $i.<br>\n";
			...
			$a[$i][$j] = new razionale(-1, 1);
			$lge[$i] = "=";
			$tmp.= "Introduziu a variável  <em>excedente</em> x<sub>$j</sub>na fila $i.<br>\n";
			if ($b[$i]->value() < 0) $base[] = array($i => $j);
		} //!strcmp( $lge[$i], ">=" )
...
function risorse_non_negative(&$a, &$b, &$lge, $numVariables, $numConstraints) {
	...
	if (isset($cambia_segno_vincolo) && count($cambia_segno_vincolo)) {
		...
		for ($i = 0; $i < count($cambia_segno_vincolo); $i++) $tmp = "A restrição numérica " . $cambia_segno_vincolo[$i] . " &egrave; multiplicado por -1 para obter o recurso positivo.<br>\n";
...
function normalizza(&$a, &$b, $i, $j, $numVariables) {
	...
	elseif ($div->num() > 0) {
		...
		$content.= "Il vincolo numero $i &egrave; diviso per " . $div->fractoa() . " a fim de não introduzir uma variável artificial desnecessária.<br>\n";
	} //$div->num() > 0
	else $content.= "<font color=\"Red\" size=\"+4\">Erro: pedido de normalização com pivot não positivo.<br></font>\n";
	return $content;
} // END normalizza()
function aggiungi_artificiali($i, $numArtificials, $numVariables, &$a, $b, &$rho, &$base) {
	...
	$base[$i - 1] = $k;
	return "Introduziu a variável artificial x<sub>$k</sub> na fila $i.<br>\n";
...
*/ {
	...
	$passo = 0;
	do {
		$content.= "<h4>Tabela para o passe $passo:</h4>\n";
		$content.= '<table summary="mostra o quadro de um lado e no\'outro o valor das variáveis" cellpadding="50%" cellspacing="50%">
 <tbody>
  <tr><td>';
		...
		if ($result == 0) {
			...
			$uscita = true;
			$content.= "Caso impossível: &rho; vai para o infinito. L\'algoritmo & egrave; mal implementado.<br>\n";
			scrivi($content);
			if (isset($grafico)) {
				delete($grafico);
				exit(0);
			...
			$content.= "Solução inelegível. O algoritmo continua a iterar.<br>\n";
			$content.= "Pivô na Linha <strong>r$i</strong> coluna <strong>x$j</strong>.<br>\n";
		}
		$passo++;
	}
	while ($uscita == false && $passo < 25);
	if ($passo == 25) $content.= "O algoritmo termina porque é; atingiu o número máximo de iterações planejadas.<br>\n";
	// ricordarsi che la soluzione in $Tableau->elemento(0,0) va interpretata col segno invertito
	// rho > 0
	$p = new razionale;
	$p = $Tableau->elemento(0, 0);
	if ($p->value() < 0) {
		$content.= 'A região de ammisssibilit&agrave; &egrave; vuota.<p><font color="red" size="+2"><strong>Não há soluções.<br></strong></font></p>';
		...
		$content.= "Caso inesperado: &rho;,por definição, não negativo, &egrave; menor que zero. o algoritmo &egrave; mal implementado.<br>\n";
		scrivi_pagina($content);
		echo "\n<!--" . memory_get_peak_usage(true) . " bytes-->" . "\n";
		if (isset($grafico)) {
			delete($grafico);
	...
	if ($Tableau->fuori_base_artificiali($numArtificials)) $content.= "Todas as variáveis artificiais estão fora da base<br>\n";
	// variabili artificiali in base nulle
	else {
		$content.= "Algumas variáveis artificiais permaneceram na base. Mais operações de pivô são necessárias.<br>\n";
		...
		for ($j = $Tableau->col - $numArtificials; $j < $Tableau->col; $j++) {
			...
			if ($arry[$j] >= 0) {
				if ($Tableau->estrai_base_artificiale($j, $arry[$j], $k, $numArtificials)) {
					$content.= sprintf("Pivô na Linha <strong>r%d</strong> coluna <strong>x%d</strong>.<br>", $arry[$j], $k);
					$content.= '<h4>Tabela para o passe' . $passo++ . ':</h4>';
					$content.= $Tableau->display_tableau();
					$content.= $Tableau->display_status(1);
				} //$Tableau->estrai_base_artificiale( $j, $arry[$j], &$k )
				else {
					$content.= '<strong>Um o pi&ugrave;variáveis artificiais permaneceram na base.</strong><br />';
					if ($p = 0)
						$content.='A solução é determinada exclusivamente.'; 
					else
					 $content.= 'Existem equações redundantes (linearmente dependentes das outras).<br> Até agora não implementei as rotinas apropriadas e o algoritmo termina.';
					// scrivi_pagina($content);
					if (isset($grafico)) {
						delete($grafico);
					} //isset( $grafico )
...
function fase_2(&$Tableau, &$content, $grafico, $name, &$image) {
	...
	$passo = 0;
	do {
		$content.= '<h4>Tabela passo a passo ' . $passo . ':</h4>';
		$content.= '
    <table summary="mostra o quadro de um lado e no outro o valor do
     variáveis" cellpadding="50%" cellspacing="50%">
    ...
    ';
		...
		if ($result == 0) {
			$uscita = true;
			if ($Tableau->unica()) {
				$content.= '<font color="red" size="+2">Soluções <strong>ÓTIMA</strong>: &nbsp;&nbsp;&nbsp; </font>';
				$content.= $Tableau->soluzione_ottima();
			} //$Tableau->unica()
			else {
				$content.= '<strong>Isto é uma das possíveis soluções ótima.</strong><br>';
				$verticeA = $Tableau->sol;
				$Tableau->altra_soluzione($i, $j, 2);
				$content.= sprintf("Pivô na Linha  <strong>r%d</strong> coluna <strong>x%d</strong>.<br>", $i, $j);
				$content.= '<h4>Tabela para o passe' . ++$passo . ':</h4>';
				$content.= '
    <table summary="Mostra o quadro de um lado e no outro o valor do
     variáveis" cellpadding="50%" cellspacing="50%">
    ...
    ';
			...
			$uscita = true;
			$content.= '<font color="red" size="+2">Solução <strong>ilimitado</strong>.<br>L\'algoritmo termina.</font><br>';
		} //$result == -1
		else {
			$content.= 'Solução melhorada. O algoritmo continua a iterar.<br>';
			$content.= sprintf("Pivô na Linha <strong>r%d</strong> coluna <strong>x%d</strong>.<br>", $i, $j);
		}
		$passo++;
	}
	while ($uscita == false && $passo < 25);
	if ($passo == 25) $content.= sprintf("O algoritmo termina porque é; atingiu o número máximo de iterações planejadas.<br>");
} // END fase_2()
function piani_di_taglio(&$Tableau, &$content) {
	$content.= '<h2>Resolução por meio do método: PIANI DI TAGLIO</h2>';
	$tagli = 0;
	$passo = 0;
	// Data una soluzione ottima, finche' non e' intera ...
	while (!$Tableau->check_intera() && $tagli < 25) {
		// aggiungi un vincolo che escluda soluzioniRisoluzione mediante metodo dei non intere
...
     variáveis" cellpadding="50%" cellspacing="50%">
     <tbody>
      ...
      <td>';
			...
			if ($result == 0) {
				...
				$uscita = true;
				$content.= '<font color="red" size="+2">Solução <strong>inadmissível</strong>.<br>O algoritmo para.</font><br>';
				scrivi_pagina($content);
				echo "\n<!--" . memory_get_peak_usage(true) . " bytes-->" . "\n";
				exit(0);
			} //$result == -1
			else {
				$content.= "Solução super excelente. O algoritmo continua a iterar.<br>\n";
				$content.= sprintf("Pivô na Linha <strong>r%d</strong> coluna <strong>x%d</strong>.<br>\n", $i, $j);
			}
			$passo++;
		}
		while ($uscita == false && $passo < 25);
	...
	if ($passo == 25) $content.= sprintf("O algoritmo termina porque é; atingiu o número máximo de iterações planejadas.<br>");
	if ($tagli == 25) $content.= sprintf("O algoritmo termina porque é; atingiu o número máximo de iterações planejadas<br>");
} // END piani_di_taglio()
///////////////////////////////////////////////////////////////////////////////
//                                                                           //
//                                 MAIN                                      //
 ...
 *
*/
$content = '<h4>O problema apresentado é:</h4>';
 ...
 *
*/
if (!strcmp($tmp, "<p></p>\n\n")) $content.= "<p><strong>O FORMULÁRIO STANDARD coincide com a representação do problema introduzido</strong>.</p>";
else {
	$content.= '<h4>O problema expresso no FORMULÁRIO PADRÃO</h4>';
	$content.= mostra_equazioni($minmax, $numVariables, $numConstraints, $c, $d, $a, $lge, $b, $intera);
 ...
 *
*/
if (!strcmp($tmp, "<p></p>\n\n")) $content.= "<p><strong>O FORMULÁRIO CANONIANO coincide com o PADRÃO</strong>.</p>";
else {
	$content.= '<h4>O problema expresso no CANON FORM</h4>';
	$content.= mostra_equazioni($minmax, $numVariables, $numConstraints, $c, $d, $a, $lge, $b, $intera);
	if (isset($verbose)) $content.= $tmp;
}
unset($tmp);
...
if ($numArtificials > 0) {
	...
	*/
	...
	$content.= '<h4>O problema expresso no FORMULÁRIO CANONIANO para a primeira fase</h4>';
