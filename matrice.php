...
class matrix {
	...
	*/
	...
	public function display_equations($fase) /* scrive la matrice $this->Tableau in forma di equazioni */ {
		...
		if (isset($intera) && !strcmp($intera, "true")) $content.= ' e TODO';
		$var = $this->col - 1;
		...
		if ($fase == 1) {
			...
			for ($j = 1; $j < $this->col; $j++) {
				...
				if ($i >= 0) { // in base?
					...
					else echo "in_base não funciona<br />\n";
	...
	*/
	...
	public function aggiungi_vincolo() /* genera e aggiunge il vincolo del metodo dei piani di taglio */ {
		...
		if ($h == - 1) return "O programa é mal implementado: não encontra restrições não completas";
		else $content = '<p>Linha com f<sub>i</sub> máxima: <strong>' . $h . '</strong></p>';
